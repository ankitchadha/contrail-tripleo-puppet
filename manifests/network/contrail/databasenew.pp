...
#  Defaults to hiera('contrail_database_node_ips')
#
class tripleo::network::contrail::databasenew(
  $step                 = hiera('step'),
  ...
  $cassandra_servers    = hiera('contrail_database_new_node_ips'),
  $disc_server_ip       = hiera('contrail::confignew::host_ip'),
  $disc_server_port     = hiera('contrail::disc_server_port'),
  $host_ip              = hiera('contrail::databasenew::host_ip'),
  $host_name            = $::hostname,
  $public_vip           = hiera('public_virtual_ip'),
  $zookeeper_client_ip  = hiera('contrail::databasenew::host_ip'),
  $zookeeper_hostnames  = hiera('contrail_database_new_short_node_names'),
  $zookeeper_server_ips = hiera('contrail_database_new_node_ips'),
)
{
  if $step == 2 {
    class {'::contrail::databasenew':
      database_params => {
        'auth_host'             => $auth_host,
